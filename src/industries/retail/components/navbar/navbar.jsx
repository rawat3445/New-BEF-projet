...
import {
  Menu, X, ChevronDown, Phone, Mail, MapPin, User, LogOut
} from 'lucide-react';
import './navbar.css';
...
const Navbar = () => {
  ...
  const solutionsItems = [
    { name: 'Revolutionize Retail', href: '#revolutionize', icon: '📱', description: 'Seamless omnichannel solutions' },
    { name: 'Bring Local to Global', href: '#local-to-global', icon: '🎨', description: 'Digital visibility for every store' },
    { name: 'Fulfillment That Delivers', href: '#fulfillment', icon: '💡', description: 'Smart logistics for growing demand' },
    { name: 'Sell Smarter with Data', href: '#sell-smarter', icon: '📱', description: 'AI-powered personalization' }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          <div className="flex-shrink-0 group">
            <h1 className="text-2xl lg:text-3xl font-black bg-gradient-to-r from-cyan-600 via-blue-600 to-indigo-600 bg-clip-text text-transparent hover:scale-105 transition-all duration-300 logo-glow-retial cursor-pointer">
              BEF Retail & E-commerce
            </h1>
            <div className="h-0.5 bg-gradient-to-r from-cyan-600 to-blue-600 scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></div>
          </div>
            ...
            {navItems.map((item) => (
              <a
                ...
                className={`relative px-4 py-2 rounded-xl text-sm font-semibold transition-all duration-300 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 hover:scale-105 ${
                  activeSection === item.id ? 'text-blue-600 bg-gradient-to-r from-blue-50 to-purple-50 nav-item-active-retail' : 'text-gray-700 hover:text-blue-600'
                }`}
              >
                {item.name}
              </a>
                  ...
                  <div className="p-2">
                    {solutionsItems.map((solution, index) => (
                      ...
                      >
                        ...
                        <div>
                          <div className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors duration-300">{solution.name}</div>
                          <div className="text-sm text-gray-600 mt-1">{solution.description}</div>
                        </div>
                      </a>
                    ))}
                  </div>
                  <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-4 border-t border-gray-100 text-xs text-center text-gray-600">
                    Need custom solutions?
                    <a href="#contact" className="text-blue-600 hover:text-purple-600 font-semibold ml-1 transition-colors duration-300">Contact us →</a>
                  </div>
                </div>
              )}
            </div>
          ...
          <div className="lg:hidden">
            <button onClick={toggleMenu} className="p-2 rounded-xl text-gray-700 hover:text-blue-600 hover:bg-gradient-to-r hover:from-blue-50 hover:to-purple-50 transition-all duration-300 hover:scale-110">
