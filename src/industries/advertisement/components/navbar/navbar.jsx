// Advertisement & PR Navbar with Auth Integration
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden lg:flex items-center space-x-4">
            {navItems.map((item) => (
              <a
                key={item.id}
                href={item.href}
                className="px-4 py-2 rounded-xl text-sm font-semibold text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition-all"
              >
                {item.name}
              </a>
            ))}
            ...
            >
              <button className="flex items-center px-4 py-2 rounded-xl text-sm font-semibold text-gray-700 hover:bg-blue-50 hover:text-blue-600">
                Solutions
                <ChevronDown className={`ml-1 h-4 w-4 transition-transform duration-300 ${dropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              {dropdownOpen && (
                <div className="absolute left-0 mt-2 w-64 bg-white rounded-2xl shadow-xl ring-1 ring-black/5 animate-scale-in">
                  <div className="p-2">
                    {solutionsItems.map((solution) => (
                      <a
                        key={solution.name}
                        href={solution.href}
                        className="flex items-start p-3 rounded-lg hover:bg-blue-50 transition-all"
                      >
            ...
            <a href="#contact" className="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-2 rounded-full text-sm font-bold hover:scale-105 transition-all">
              Contact Us
            </a>
              ...
              {isAuthenticated && user ? (
                <div className="relative">
                  <button
                    ...
                    className="flex items-center gap-2 bg-[linear-gradient(to_right,var(--primary-blue),var(--accent-cyan))] text-white px-6 py-2 rounded-full font-semibold hover:shadow-lg transition-all hover:scale-105"
                  >
                    {user.image ? (
                  ...
                  {showUserDropdown && (
                    <div className="absolute right-0 mt-2 w-64 bg-white rounded-lg shadow-lg border border-gray-200 py-2 z-50">
                      ...
                      <button
                        ...
                        className="w-full px-4 py-2 text-left hover:bg-gray-50 flex items-center gap-2 text-red-600 hover:text-red-700 transition-colors"
                      >
          ...
          <div className="lg:hidden">
            <button onClick={() => setIsOpen(!isOpen)} className="p-2 rounded-lg text-gray-700 hover:bg-blue-50 hover:text-blue-600">
